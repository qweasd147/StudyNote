...
Foo에 apply와 bind가 존재하는 이유는 `Foo.__proto__` 가 `Function.prototype`을 바라보기 때문이다.
...
정리하면 Foo 의 prototype(원형)은 Function이고, Function의 prototype은 Object가 된다. 따라서 Foo의 'hasOwnProperty'
