...
인터페이스를 작성하는 방식이다.  java 1.8이상부터 이렇게 별 의미없고
...
또한 매개변수가 2개인 함수형 인터페이스는 위 인터페이스 명 앞에 Bi가 붙는다(BiFunction<T,U,R>).
...
ClassName::MethodName 형태로 작성하고, ClassName은 static이건 아니건 상관없다.

위 소스를 보면 useReference 메소드에서 넘겨받는 파라미터는 generics에 의해 개수와 타입이

제한되어 맵핑되는 것을 알수 있다. 근데 개인적으로 메소드 참조는 뭔가
